...
public class OAuth2SecurityConfiguration {
	...
	protected static class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		@Autowired
		private PotlatchUserDetailsService userDetailsService;
		...
		protected void registerAuthentication(
			...
			auth.userDetailsService(new PotlatchUserDetailsService(users));
		}
	}
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
					...
					.scopes("read","write").resourceIds("video","image")
					.and()
					// Create a second client that only has "read" access to the
					// video service
					.withClient("mobileReader").authorizedGrantTypes("password")
					.authorities("ROLE_CLIENT")
					.scopes("read").resourceIds("video","image")
					.accessTokenValiditySeconds(3600).and().build();

			// Create a series of hard-coded users. 
			/*UserDetailsService svc =new PotlatchUserDetailsManager(
					Arrays.asList(
							User.create("admin", "pass", "ADMIN", "USER"),
							User.create("user0", "pass", "USER"),
							User.create("tempuser", "", "USER")
							));*/
			UserDetailsService svc=new PotlatchUserDetailsService();
		...
		@Bean 
	    public UserDetailsService potlatchDetailsService() {
	        return new PotlatchUserDetailsService();     
	    } 
		/**
